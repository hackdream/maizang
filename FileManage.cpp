...
//DWORD CFileManage::ListDriver()
{
	...
	HTREEITEM hTreeRoot = This->m_FileTree.GetRootItem();//得到最高那个条目即显示我的电脑的那个条目
	...
	for(DWORD i = 0; i < dwNum; i++)
	{
		...
		//插入数据
		HTREEITEM hItem = This->m_FileTree.InsertItem(pInfo[i].display,iImage,iSelectedImage, hTreeRoot,0);
		This->m_FileTree.SetItemData(hItem, (DWORD)pInfo[i].driver[0]);
	}
	
	This->m_FileTree.Expand(hTreeRoot,TVE_EXPAND);
...
void CFileManage::OnSelchangedFiletree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	// TODO: Add your control notification handler code here
		...
		if(hSelItem == m_FileTree.GetRootItem()) return;//选中的是我的电脑 没有必要显示响应的文件 因为没有
		
		DWORD ch = m_FileTree.GetItemData(hSelItem);//ch保存选中的磁盘
    ...
    //	m_hWorkThread = ThreadTemplate::StartThread<CFileManage, DWORD>(this, ListDirectory);
					unsigned dwThreadId;
			m_hWorkThread  =
			 (HANDLE)_beginthreadex(NULL,				 
									 0,					 
